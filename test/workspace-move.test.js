...
function mockState() {
    const ws1 = mockWorkspace(1);
    const ws2 = mockWorkspace(2);
...
describe('workspace move detection', () => {
    ...
    it('ignores transient child windows and fullscreen windows', () => {
        const state = mockState();
        const child = mockWindow({
            workspace: state.ws2,
            transientFor: mockWindow({id: 99, workspace: state.ws1}),
        });
        const fullscreen = mockWindow({workspace: state.ws2, fullscreen: true});
        state.previousWorkspaces.set(child, state.ws1);
        state.previousWorkspaces.set(fullscreen, state.ws1);

        assert.equal(
            isManualCrossWorkspaceMove(child, state.ws2, moveContext(state)),
            false
        );
        assert.equal(
            isManualCrossWorkspaceMove(fullscreen, state.ws2, moveContext(state)),
            false
        );
    });
});
